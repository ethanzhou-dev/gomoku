(() => {
  ...
  var getShapeFast = (board, x, y, offsetX, offsetY, role) => {
    if (board[x + offsetX + 1][y + offsetY + 1] === 0 && board[x - offsetX + 1][y - offsetY + 1] === 0 && board[x + 2 * offsetX + 1][y + 2 * offsetY + 1] === 0 && board[x - 2 * offsetX + 1][y - 2 * offsetY + 1] === 0) {
  ...
  var isLine = function(a, b, size) {
    ...
    return x1 === x2 && Math.abs(y1 - y1) < maxDistance || y1 === y2 && Math.abs(x1 - x2) < maxDistance || Math.abs(x1 - x2) === Math.abs(y1 - y2) && Math.abs(x1 - x2) < maxDistance;
  };
  ...
  var Evaluate = class {
    constructor(size = 15) {
      this.size = size;
      this.board = Array.from({ length: size + 2 }).map(
        (_, i) => Array.from({ length: size + 2 }).map(
          (_2, j) => i === 0 || j === 0 || i === size + 1 || j === size + 1 ? 2 : 0
        )
      );
    ...
    initPoints() {
      ...
      for (let role of [1, -1]) {
        ...
        for (let key of Object.keys(shapes)) {
          ...
          this.pointsCache[role][shape] = /* @__PURE__ */ new Set();
        }
      }
    }
    // 只返回和最后几步在一条直线上的点位。
    ...
    getPointsInLine(role) {
      ...
      Object.keys(shapes).forEach((key) => {
        pointsInLine[shapes[key]] = /* @__PURE__ */ new Set();
      });
      let last2Points = this.history.slice(-config.inlineCount).map(([position, role2]) => position);
      const processed = {};
    ...
    getPoints(role, depth, vct2, vcf2) {
      ...
      if (config.onlyInLine && this.history.length >= config.inlineCount) {
        ...
        if (pointsInLine) {
          performance.getPointsTime += /* @__PURE__ */ new Date() - start;
          return pointsInLine;
        }
      }
      let points = {};
      Object.keys(shapes).forEach((key) => {
        points[shapes[key]] = /* @__PURE__ */ new Set();
      });
      const lastPoints = this.history.slice(-4).map(([position, role2]) => position);
      for (let r of [role, -role]) {
    ...
    updatePoint(x, y) {
      ...
      performance.updateTime += /* @__PURE__ */ new Date() - start;
    }
    /*
     计算单个点的得分
     计算原理是：
     ...
     */
    updateSinglePoint(x, y, role, direction = void 0) {
    ...
    _getMoves(role, depth, onlyThree = false, onlyFour = false) {
      ...
      if (fives?.size || blockFives?.size) return /* @__PURE__ */ new Set([...fives, ...blockFives]);
      const fours = points[shapes.FOUR];
      const blockfours = points[shapes.BLOCK_FOUR];
      if (onlyFour || fours?.size) {
        return /* @__PURE__ */ new Set([...fours, ...blockfours]);
      }
      const four_fours = points[shapes.FOUR_FOUR];
      if (four_fours.size) return /* @__PURE__ */ new Set([...four_fours, ...blockfours]);
      const threes = points[shapes.THREE];
      const four_threes = points[shapes.FOUR_THREE];
      if (four_threes?.size) return /* @__PURE__ */ new Set([...four_threes, ...blockfours, ...threes]);
      const three_threes = points[shapes.THREE_THREE];
      if (three_threes?.size) return /* @__PURE__ */ new Set([...three_threes, ...blockfours, ...threes]);
      if (onlyThree) return /* @__PURE__ */ new Set([...blockfours, ...threes]);
  ...
  var Board = class _Board {
    ...
    reverse() {
      const newBoard = new _Board(this.size, -this.firstRole);
  ...
  var cache = new Cache();
  var factory = (onlyThree = false, onlyFour = false) => {
  ...
  var minmax = (board, role, depth = 4, enableVCT = true) => {
    if (enableVCT) {
  ...
  // src/adapter.js
  self.importScripts("rules.js");
  var originalGetValuableMoves = board_default.prototype.getValuableMoves;
  self.onmessage = function(e) {
    const { board: inputBoard, depth, aiRole, forbidden, isHint } = e.data;
    const size = inputBoard.length || 15;
    board_default.prototype.getValuableMoves = function(role, depthLevel, onlyThree, onlyFour) {
      let moves = originalGetValuableMoves.call(this, role, depthLevel, onlyThree, onlyFour);
      if (forbidden && role === 1) {
        let currentBoard = [];
        for (let i = 0; i < size; i++) {
          currentBoard.push([]);
          for (let j = 0; j < size; j++) {
            ...
            currentBoard[i][j] = v === 1 ? 1 : v === -1 ? 2 : 0;
          }
        }
        moves = moves.filter((pos) => {
          const r = pos[0];
          const c = pos[1];
          if (typeof self.GomokuRules !== "undefined" && self.GomokuRules.checkForbidden) {
            return !self.GomokuRules.checkForbidden(currentBoard, r, c, size);
          }
          return true;
        });
        if (moves.length === 0) {
          return originalGetValuableMoves.call(this, role, depthLevel, onlyThree, onlyFour);
        }
      }
      return moves;
    };
    const gameBoard = new board_default(size);
    for (let i = 0; i < size; i++) {
      for (let j = 0; j < size; j++) {
        if (inputBoard[i][j] === 1) {
          gameBoard.put(i, j, 1);
        } else if (inputBoard[i][j] === 2) {
          gameBoard.put(i, j, -1);
        }
      }
    }
    const roleToPlay = aiRole === 1 ? 1 : -1;
    gameBoard.role = roleToPlay;
    try {
      const res = minmax(gameBoard, roleToPlay, depth);
      const move = res[1];
      if (move) {
        self.postMessage({ r: move[0], c: move[1], isHint });
      } else {
        fallback(inputBoard, size, isHint);
      }
    } catch (err) {
      console.error("AI Error:", err);
      fallback(inputBoard, size, isHint);
    }
    board_default.prototype.getValuableMoves = originalGetValuableMoves;
  };
  function fallback(inputBoard, size, isHint) {
    for (let i = 0; i < size; i++) {
      for (let j = 0; j < size; j++) {
        if (inputBoard[i][j] === 0) {
          self.postMessage({ r: i, c: j, isHint });
          return;
        }
      }
    }
    self.postMessage({ r: -1, c: -1, isHint });
  }
})();
